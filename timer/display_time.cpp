		...
		case COUNTER_MOD:
				...
				{//если таймер включен то отображаем либо время до выключения (и тогда мигают точки)
				 //либо время до следующего включения - точки не мигают
					...
					//Отображение состояния будильника
					// Если текущее время больше стартового то 
					// А) если будильник работает то отображаем время до конца работы
					// Б) если будильник не работает то отображаем время до начала работы
...
//TODO: Изменить функцию так чтоб можно было поставить период 24 часа 00 минут
...
void UpdateTime()
{
	...
	{	
		uint8_t result = 1;
//0 - нормальная работа, время получается от RTC
//1 - тестирование, время инкрементируется при каждом обновлении
#if 0
